...
import Char
import CommonParser exposing (lineEnd, newLine, number, spaces, stringLiteral)
import Dict exposing (Dict)
import List
import List.Extra
import Parser exposing ((|.), (|=), Parser)
import Pcfg.Grammar exposing (Form(..))
import Set
import Tuple
...
definitions =
    Parser.loop
        ...
        |> Parser.andThen
            (\formDefinitions ->
                if List.Extra.allDifferent <| List.map Tuple.first formDefinitions then
                    Parser.succeed <| Dict.fromList formDefinitions

                else
                    Parser.problem "forms may only be defined once"
            )
        ...
        |> Parser.andThen
            (\string ->
                if String.isEmpty string then
                    Parser.problem "An unquoted literal form may not be empty"

                else
                    Parser.succeed <| LiteralForm string
            )
...
unionForm =
    Parser.loop
        ...
        |> Parser.andThen
            (\weightedForms ->
                if List.isEmpty weightedForms then
                    Parser.problem
                        "at least one clause is necessary in a weighted union form"

                else
                    Parser.succeed <| UnionForm weightedForms
            )
